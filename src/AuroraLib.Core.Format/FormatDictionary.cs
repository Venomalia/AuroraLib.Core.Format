...
{
    /// <summary>
    /// Represents a collection of <see cref="IFormatInfo"/>, provides lookup
    /// and identification of custom file formats based on stream content
    /// and file extensions.
    /// </summary>
    [DebuggerDisplay("{Count}")]
    public sealed class FormatDictionary : ICollection<IFormatInfo>, IReadOnlyDictionary<string, IFormatInfo>, IReadOnlyCollection<IFormatInfo>
    {
        ...
        {
            if (!TryAdd(item))
                throw new ArgumentException($"Item with MIME type '{item.MIMEType}' already exists.");
        }
        ...
        void ICollection<IFormatInfo>.CopyTo(IFormatInfo[] array, int arrayIndex)
        {
            ...
            if (arrayIndex < 0) throw new ArgumentOutOfRangeException(arrayIndex.ToString(), arrayIndex, $"{arrayIndex} cannot be negative.");
